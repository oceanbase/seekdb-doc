...
この例では、`query()` と比較して `hybrid_search()` の利点を示します。

`hybrid_search()` には主に以下の利点があります：

* フルテキスト検索とベクトル類似性検索を同時に実行できる

* フルテキスト検索とベクトル検索に対して、それぞれフィルター条件を設定できる

* Reciprocal Rank Fusion アルゴリズムにより、2種類の検索のランキング結果を統合し、関連性を向上させる

* `query()` では対応できない複雑なシナリオを処理できる
...
* pyseekdbがサポートするAPIについては、[APIリファレンス](../50.apis/10.api-overview.md)を参照してください。

* [Simpleインデックスの例](10.pyseekdb-simple-sample.md)

* [Completeインデックスの例](50.pyseekdb-complete-sample.md)
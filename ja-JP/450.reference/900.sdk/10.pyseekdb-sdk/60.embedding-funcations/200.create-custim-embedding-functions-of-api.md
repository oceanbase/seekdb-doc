...
`EmbeddedFunction` プロトコルを実装することで、カスタム埋め込み関数を作成できます。この機能には以下の内容が含まれます：

* `__call__` メソッドを実装します。このメソッドは `Documents (str or List[str])` を受け取り、`Embeddings (List[List[float]])` を返す必要があります。

* ベクトルの次元を返すために、オプションで次元属性を実装します。
...
* `__call__` メソッドを実装する：
  ...
  * すべてのベクトルは同じ次元を持たなければなりません。
  * 引数：単一または複数のドキュメントの型は、strまたはList[str]です。
  * 戻り値：埋め込みベクトルのフィールド型は、`List[List[float]]`です。

* (推奨)dimensionプロパティを実装する：
  * 戻り値：この関数が生成するベクトルの型は `int` です。
  * collectionsの作成は、一意性の検証に役立ちます。

* 特殊なケースを処理する
  * 単一の文字列入力はリストに変換する必要があります。
  * 空の入力は空のリストを返す必要があります。
  * 出力されるすべてのベクトルは同じ次元を持たなければなりません。

## 例1：文章変換ツールのカスタム埋め込み機能

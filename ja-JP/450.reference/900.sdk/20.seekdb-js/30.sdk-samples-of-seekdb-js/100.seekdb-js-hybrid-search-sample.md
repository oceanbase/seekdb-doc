...
この例では、`query()` と比較して `hybridSearch()` の利点を示します。

`hybridSearch()` には主に以下の利点があります：

* フルテキスト検索とベクトル類似性検索の同時実行をサポートします。

* フルテキスト検索とベクトル検索にそれぞれフィルター条件を設定できます。

* Reciprocal Rank Fusion アルゴリズムにより、2種類の検索のランキング結果を統合し、関連性を向上させます。

* `query()` では対応できない複雑なシナリオを処理します。

## 手順

1. seekdbに接続します。
2. Embedding Functionsを持つcollectionを作成します。
3. documentsを使用してデータを追加します（ベクトルは自動生成されます）。
4. `query()` と `hybridSearch()` を使用してクエリを実行し、結果を比較します。
5. 結果を出力します。
...
* [Simple サンプル](10.seekdb-js-simple-sample.md)

* [Complete サンプル](50.seekdb-js-complete-sample.md)
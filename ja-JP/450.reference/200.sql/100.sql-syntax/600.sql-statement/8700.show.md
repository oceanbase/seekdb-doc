...
|     パラメータ          |     説明         |
|--------|--------|
|\[EXTENDED\] \[FULL\] TABLES \{FROM \| IN} database_name   | `database_name`データベース内のすべてのテーブルを表示します。 <ul><li>`EXTENDED` キーワードは隠れたテーブルを表示するために使用されます。seekdbの現在のバージョンでは隠れたテーブルが生成されないため、このパラメータは実質的な効果はありません。</li><li>`FULL` キーワードはテーブルタイプを表示するために使用されます。</li></ul>|
...
| \[FULL\] PROCESSLIST     | プロセスリストを表示します。詳細は以下のとおりです： <ul><li><code>SHOW PROCESSLIST</code> は、簡略化されたプロセスリストを表示します。詳細は以下のとおりです：  <ul><li><code>Id</code>：プロセスID、すなわち現在のセッションのクライアントセッションID。このIDはクライアント上のセッションを一意に識別するものです。</li><li><code>User</code>：接続ユーザー名。</li><li><code>Host</code>：クライアント側のIPアドレスとポート。</li><li><code>db</code>：アクセスしたデータベース名。</li><li><code>Command</code>：現在実行中のコマンドタイプ。<code>Query</code>、<code>Sleep</code>などが取り得ます。</li><li><code>Time</code>：現在のコマンド実行時間（秒単位）。コマンドが再試行された場合、時間はリセットされて再計算されます。</li><li><code>State</code>：現在のセッション状態。<code>SLEEP</code>、<code>ACTIVE</code>などが取り得ます。</li><li><code>Info</code>：現在実行中のコマンドを表示します。長さは100文字に制限され、それを超える部分は切り捨てられます。</li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code>：完全なプロセスリストを表示します。各プロセスの詳細情報を含みます。詳細は以下のとおりです：  <ul><li><code>Id</code>：プロセスID、すなわち現在のセッションのクライアントセッションID。このIDはクライアント上のセッションを一意に識別するものです。</li><li><code>User</code>：接続ユーザー名。</li><li><code>Tenant</code>：接続テナント。</li><li><code>Host</code>：クライアント側のIPアドレスとポート。</li><li><code>db</code>：アクセスしたデータベース名。</li><li><code>Command</code>：現在実行中のコマンドタイプ。<code>Query</code>、<code>Sleep</code>などが取り得ます。</li><li><code>Time</code>：現在のコマンド実行時間（秒単位）。コマンドが再試行された場合、時間はリセットされて再計算されます。</li><li><code>State</code>：現在のセッション状態。<code>SLEEP</code>、<code>ACTIVE</code>などが取り得ます。</li><li><code>Info</code>：現在実行中のコマンドを表示します。</li><li><code>Ip</code>：サーバーのIPアドレス。</li><li><code>Port</code>：SQLポート番号。</li></ul>  </li></ul>  |
...
* `SHOW COLUMNS` を使用してテーブル `tbl2` の列情報を確認します。
...
* `SHOW EXTENDED COLUMNS` を使用してテーブル `tbl2` の隠れた列情報を確認します。
...
* `SHOW INDEX` を使用してテーブル `tbl2` のインデックス情報を確認します。
...
* `SHOW EXTENDED INDEX` を使用してテーブル `tbl2` の隠れたインデックス情報を確認します。

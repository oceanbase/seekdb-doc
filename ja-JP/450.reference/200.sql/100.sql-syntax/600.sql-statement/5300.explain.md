...
| SET statement_id | 現在のクエリに文字列タグを設定し、後からそのSQLの実行計画情報を検索しやすくするために使用されます。`SET statement_id` を指定しない場合、デフォルトで空文字列がタグとして使用されます。 |
| PRETTY \| PRETTY_COLOR | 実行計画の木構造において、親ノードと子ノードを線または色付きの線で結び、実行計画をより読みやすく表示します。|
| BASIC                         | 出力する計画の基本情報を指定します。例えば、演算子ID、演算子名、参照するテーブル名などです。   |
| OUTLINE                       | 出力する計画情報に `OUTLINE` 情報を含めることを指定します。 |
| EXTENDED                      | `EXPLAIN` が追加情報を生成します。これには以下が含まれます：各演算子の入力列と出力列、アクセスするテーブルのパーティション情報、現在使用中のフィルター情報、現在の演算子がインデックスを使用している場合は、使用されているインデックス列と抽出されたQuery Rangeを表示します。 |
| EXTENDED_NOADDR               | 追加情報を簡潔な方法で表示します。   |
| PARTITIONS                    | パーティション関連情報を表示します。  |
| FORMAT = \{TRADITIONAL\| JSON\} | `EXPLAIN` の出力形式を指定します： <ul><li> `TRADITIONAL`：テーブル出力形式    </li> <li> `JSON`：`KEY:VALUE` 出力形式で、JSONは `EXTENDED` および `PARTITIONS` 情報を含むJSON文字列として表示されます。</li></ul>    |
...
* [SQL実行計画の概要](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013759)
* [SQL実行計画](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013986)
* [実行計画](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002013516)
...
このセクションでは、SQL文の汎用構文について説明します。これには、定数、文字セット、データ型、SQL属性、ストレージ属性が含まれます。

## 定数

* `INT_VALUE`：整数型定数に一致します。例：`123`。

* `DECIMAL_VALUE`：固定小数点数型定数に一致します。例：`123.456`。

* `STR_VALUE`：文字列定数に一致します。例：`abc`。

* `NULL`：`NULL` 定数に一致します。

* `STORAGE_SIZE`：ストレージ長定数に一致します。整数形式のデフォルト単位はバイトですが、文字列形式では単位を M または G として指定できます。例：`1024` または `'500M'`。
...
* `TABLET_SIZE`：並列メジャーコンパクション時の単一タスクの最小シャーディングサイズを指定します。

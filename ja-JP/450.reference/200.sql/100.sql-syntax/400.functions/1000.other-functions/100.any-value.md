...
以下の例では、`name` が非インデックス列の場合、`ONLY_FULL_GROUP_BY SQL` モードを有効にすると、`GROUP BY` クエリは失敗します。これは、`city` が非集約列であるため、各 `name` グループ内の `city` 値が不確定だからです。
...
`GROUP BY` 句がない場合、グループが1つしかない状態でどの `name` 値を選択するかが不確定であるため、クエリは失敗します。しかし、`ANY_VALUE()` はこの問題を回避し、集約関数を参照するクエリをデータベースが受け入れることを可能にします。

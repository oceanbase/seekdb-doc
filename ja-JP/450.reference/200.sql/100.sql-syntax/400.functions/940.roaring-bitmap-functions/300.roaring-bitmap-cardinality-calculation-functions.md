...
`rb_and_cardinality()` 関数は、2つのビットマップデータにAND演算を施した後の新しいビットマップデータのカードナリティを返します。構文は以下のとおりです：
...
`rb_and_null2empty_cardinality()` 関数の計算ロジックは `rb_and_cardinality()` と同じですが、空の入力を空のビットマップデータと見なします。
...
## rb_or_cardinalityとrb_or_null2empty_cardinality

`rb_or_cardinality()` 関数は、2つのビットマップデータにOR演算を施した後の新しいビットマップデータの基数を返します。構文は以下のとおりです：
...
`rb_xor_cardinality()` 関数は、2つのビットマップデータにXOR演算を施した後の新しいビットマップデータの基数を返します。構文は以下のとおりです：
...
この関数は、パラメータ `rb1` を計算基準として、それと `rb2` の差を計算します。つまり、ビットマップデータ値 `rb1 - rb2` のカードナリティを返します。
...
`rb_or_cardinality_agg()` 関数は、ビットマップ列の和集合のカードナリティを計算するために使用されます。構文は以下のとおりです：
...
`rb_and_cardinality_agg()` 関数は、ビットマップ列の共通要素の基数を計算します。構文は以下のとおりです：

...
* `json_value_on_empty_clause`：Pathフィルタリング後のデータがNULL値の場合のユーザー選択可能な期待される動作です。選択可能な動作には3種類あります：`NULL`、`ERROR`、デフォルト値 `DEFAULT`。

  * `NULL ON EMPTY`：この列を `NULL` に設定します。これがデフォルトの動作です。
  * `DEFAULT json_string`：`json_string` を解析して、JSONオブジェクトまたは配列のデフォルト値として代わりに設定します。seekdbでは、デフォルト値は任意の型の定数データとして許可されています。

...
この関数はTable Functionフレームワークに組み込まれた関数であり、Table Function内から呼び出すことが可能で、最終的にN行のデータを返します。Nは0以上の64ビットの正の整数です。

Table FunctionはSQL言語で使用される関数の一種で、結果としてデータテーブルを返すことができます。従来のSQL関数がスカラー値しか返せないのとは異なり、Table Functionは複数行・複数列のデータセットを返すことができます。

`TABLE(GENERATOR(N))` は実際にはデータを生成するわけではなく、連続したN行の「行」を生成します。データを生成する必要がある場合は、`SELECT` の後に定数、変数、関数、UDFなどの具体的な式を追加する必要があります。`TABLE(GENERATOR(N))` は並列実行に利用できます。例えば、並列 `SELECT` や並列DMLなどです。ただし、`TABLE(GENERATOR(N))` が生成する行を読み取るスレッドは1つだけであり、`TABLE(GENERATOR(N))` を含むDFOの `DOP=1` となります。

## 例

以下の例では、`GENERATOR()` 関数を使用して条件に合致するランダムな数値を生成します。

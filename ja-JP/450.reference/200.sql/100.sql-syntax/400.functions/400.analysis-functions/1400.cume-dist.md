...
この関数は、行のグループ（パーティション）内での累積分布値を計算し、`0` より大きく `1` 以下の値を返します。ソート後の現在の行の位置（そのデータ以下の行数）を、パーティション内の全行数で割ることで、行の累積分布値を決定します。
...
3. `job_id` でグループ化し、給与の昇順で並べ替えた場合、各行データがウィンドウ内でのソート列における割合を求めます。

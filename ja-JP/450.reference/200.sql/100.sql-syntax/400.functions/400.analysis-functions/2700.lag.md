...
1回のクエリで、現在行の同じフィールドの次の `offset` 行のデータを取得する操作は、同じテーブルの自己結合でも実現できますが、`LAG()` ウィンドウ関数の方が効率的です。
...
| query_partition_clause         | クエリのパーティションを指定するために使用されます。 デフォルトではグローバルデータが使用されます。                                                     |

...
seekdbは、数字 `1,2,...` を用いて関係 `t` の1列目、2列目などをそれぞれ指すことをサポートしています。したがって、上記のクエリは以下のように書き換えることもできます：
...
`GROUP BY` 句は、クエリ結果をグループ化するために使用されます。グループ化列 `c1,c2,...` を複数指定することで、クエリ結果をグループ化列の順にグループ分けすることができます。

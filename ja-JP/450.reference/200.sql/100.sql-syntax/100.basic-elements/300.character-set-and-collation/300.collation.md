...
seekdbのデフォルトの照合順序は`utf8mb4_general_ci`です。

seekdbが現在サポートしている照合順序は、次のテーブルのとおりです。
...
| utf8mb4_unicode_520_ci  | utf8mb4 | Unicode 5.2.0の照合順序を使用します。Unicodeコードポイントでソートし、大文字と小文字の区別は行いません。|
| utf8mb4_croatian_ci     | utf8mb4 | クロアチア語(Croatian)の照合順序を使用します。`utf8mb4_croatian_ci`は`utf8_croatian_ci`と互換性があります。|
| utf8mb4_czech_ci        | utf8mb4 | チェコ語(Czech)の照合順序を使用します。|
...
| latin1_spanish_ci  | latin1  | スペイン(Spanish)言語環境で使用される照合順序。|
| latin1_bin         | latin1  | latin1はバイナリソート規則を使用します。|
| gb18030_2022_bin        | gb18030_2022 | バイナリソート規則を使用します。 |
| gb18030_2022_chinese_ci | gb18030_2022 | ピンインソート規則を使用します。大文字小文字を区別しません。MySQLモードにおけるこの文字セットのデフォルトの照合順序です。 |
| gb18030_2022_chinese_cs | gb18030_2022 | ピンインソート規則を使用します。大文字小文字を区別します。  |
| gb18030_2022_radical_ci | gb18030_2022 | 部首・筆順ソート規則を使用します。大文字小文字を区別しません。|
| gb18030_2022_radical_cs | gb18030_2022 | 部首・筆順ソート規則を使用します。大文字小文字を区別します。  |
...
| gb2312_chinese_ci   | gb2312       | GB2312文字セットを使用し、中国語ソート規則に従って大文字小文字を区別しないソートを行います。|
| gb2312_bin          | gb2312       | GB2312文字セットを使用し、バイナリ順序で大文字小文字を区別したソートを行います。|
| ujis_japanese_ci    | ujis         | UJIS文字セットを使用し、日本語ソート規則に従って大文字小文字を区別しないソートを行います。|
| ujis_bin            | ujis         | UJIS文字セットを使用し、バイナリ順序で大文字小文字を区別したソートを行います。|
| euckr_korean_ci     | euckr        | EUCKR文字セットを使用し、韓国語ソート規則に従って大文字小文字を区別しないソートを行います。|
| euckr_bin           | euckr        | EUCKR文字セットを使用し、バイナリ順序で大文字小文字を区別したソートを行います。|
| eucjpms_japanese_ci | eucjpms      | EUCJPMS文字セットを使用し、日本語ソート規則に従って大文字小文字を区別しないソートを行います。|
| eucjpms_bin         | eucjpms      | EUCJPMS文字セットを使用し、バイナリ順序で大文字小文字を区別したソートを行います。|
| cp932_japanese_ci   | cp932        | CP932文字セットを使用し、日本語ソート規則に従って大文字小文字を区別しないソートを行います。|
...
| hp8_english_ci      | hp8          | HP8文字セットを使用し、英語ソート規則に従って大文字小文字を区別しないソートを行います。|
| hp8_bin             | hp8          | HP8文字セットを使用し、バイナリ順に大文字小文字を区別してソートします。|
| macroman_general_ci | macroman     | MacRoman文字セットを使用し、一般的なソート規則に従って大文字小文字を区別せずにソートします。|
| macroman_bin        | macroman     | MacRoman文字セットを使用し、バイナリ順に大文字小文字を区別してソートします。|
| swe7_swedish_ci     | swe7         | SWE7文字セットを使用し、スウェーデン語のソート規則に従って大文字小文字を区別せずにソートします。|
| swe7_bin            | swe7         | SWE7文字セットを使用し、バイナリ順に大文字小文字を区別してソートします。|

  :::info

  任意のUnicode文字セットについて、`xxx_general_ci`ソート規則を使用した操作は、`xxx_unicode_ci`ソート規則を使用した操作よりも高速です。

  照合順序の変更は現在サポートされていません。
...
* 各文字セットにはデフォルトの照合順序が設定されています。`SHOW CHARACTER SET`ステートメントは、各文字セットのデフォルトの照合順序を示します。`SHOW COLLATION`ステートメントには、各照合順序がその文字セットのデフォルト値かどうかを示す列があります（デフォルトの場合はYes、そうでない場合は空文字列）。

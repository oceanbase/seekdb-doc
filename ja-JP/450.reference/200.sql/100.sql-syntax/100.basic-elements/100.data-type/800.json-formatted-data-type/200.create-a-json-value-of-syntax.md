...
オブジェクトの構造は、一対の大括弧で囲まれた0個以上の名前/値のペア（またはメンバー）で表されます。オブジェクト内の名前は一意である必要があります。名前は文字列であり、各名前の後にはコロンが続き、名前と値を区切ります。複数の名前/値を区切る場合は、単一のカンマを使用します。
例：
...
配列の構造は、角カッコで囲まれた0個以上の値（要素とも呼ばれる）で表されます。配列の要素はカンマで区切られ、配列内の値がすべて同じである必要はありません。
...
数字は十進数形式を使用し、整数部を含みます。必要に応じて、接頭辞としてマイナス記号（-）を付けることができ、その後に分数部と/または指数部を続けることができます。先頭のゼロは許可されません。小数部は、小数点の後に1文字以上の数字が続く形式です。指数部は大文字または小文字のEで始まり、その後にプラス記号（+）またはマイナス記号（-）を続けることができます。Eと選択可能な符号の後には、1文字以上の数字を続けることができます。
...
1. セミ構造化エンコーディングの有効化
   ...
   セミ構造化エンコーディング機能を有効にする場合は、パラメータ[micro_block_merge_verify_level](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971939)がデフォルト設定の`2`であることを必ず確認してください。マイクロブロックマージ検証を無効にしないでください。
   ...
   その他の構文については、[CREATE TABLE](../../../600.sql-statement/2600.create-table.md)を参照してください。
...
3. セミ構造化エンコーディング設定の確認
   ...
   `semistruct_encoding_type=encoding'`の場合にのみ、このパラメータ情報がクエリに表示され、セミ構造化エンコーディング機能が有効であることを示します。

セミ構造化エンコーディングを使用すると、[JSON_VALUE() 関数](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001975890)の条件フィルタリングクエリのパフォーマンスを向上させることができます。SEEKDBは、JSONセミ構造化エンコーディング技術に基づき、`JSON_VALUE`式を使用した条件フィルタリングクエリのパフォーマンスを最適化しています。JSONデータがサブ列に分割されているため、システムはエンコード済みのサブ列データに直接基づいてフィルタリングを実行でき、完全なJSON構造を復元する必要がなくなり、クエリ効率が大幅に向上します。

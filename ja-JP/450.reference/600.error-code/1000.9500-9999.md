...
* エラー原因：自治トランザクション句の位置が間違っています。最初のBlockでのみ使用できます。
...
* エラー原因：`EXIT`または`CONTINUE`文は、ループ体内で使用する必要があります。

* 解決策：`EXIT`または`CONTINUE`句の使用が正しいか確認します。
...
* エラー原因：一つの Object 型では、`MAP` または `ORDER` 関数のどちらか一方を一つだけ定義できます（`MAP` と `ORDER` の両方を同時に定義することはできません）。
...
* エラー原因：コンストラクタ関数の名前は型名と同じでなければなりません。

* 解決策：両者が同じであることを確認してください。
...
* エラー原因：`MAP` 関数はスカラー値を返す必要があり、複雑な型は返せません。
...
* エラー原因：`MAP` 関数のパラメータは、`SELF` パラメータを1つだけ持たなければなりません（`SELF` パラメータは明示的または暗黙的に定義できます）。
...
* エラー原因：`$ERROR` ステートメントによって生成された例外です。想定された例外です。
...
* エラー原因：サブプロシージャを呼び出す際に適切な関数が見つからなかった。関数名が間違っている、パラメータの数が間違っている、またはパラメータの型がマッチしない可能性があります。

* 解決策：関数名、パラメータの数、パラメータの型を確認します。それでも問題がある場合は、関数のアクセス範囲を確認してください。
...
* エラー原因：複数の関数呼び出しに一致します。呼び出した関数に複数のマッチする呼び出し方法が存在し、正確に一致する関数がない可能性があります。
...
* エラー原因：condition 定義に重複項目があります。

* 解決策：condition 定義から重複項目を削除します。
...
* エラー原因：増分更新マテリアライズドビューの作成に関する基本要件を満たしていません。例：
  ...
  * 増分更新マテリアライズドビューの定義に集合演算子が含まれていません。
...
* エラー原因：マテリアライズドビューの定義がリアルタイムマテリアライゼーションの要件を満たしていません。詳細な原因については、`$detail` の部分を参照してください。
...
* 解決策：マテリアライズドビューをフル更新します。

## ERROR 20000 (HY000) : The stored procedure 'raise_application_error' was called which causes this error to be generated","-%05ld: %.\*s

* seekdb エラーコード：20000

* エラー原因：`RAISE_APPLICATION_ERROR`プロシージャ呼び出しによって生成されたユーザー定義例外です。このエラーコードは隠れたコードであり、実際のエラーコードはユーザーが指定したものに準じます。プロシージャ`RAISE_APPLICATION_ERROR`で指定できるユーザーエラーコードの範囲は20000から20999です。
...
* エラー原因：`RAISE_APPLICATION_ERROR` で生成されるユーザー定義例外のエラーコード範囲が正しくありません。ユーザーが指定するエラーコードは、20000 から 20999 の範囲内である必要があります。

* 解決策：エラーコードの範囲を 20000 から 20999 に調整します。

## ERROR 32491 (HY000) : snapshot expression not allowed here

* seekdb エラーコード：32491

* エラー原因：ここでは SNAPSHOT 式を使用できません。SNAPSHOT 式は読み取り専用ステートメントでのみサポートされています。

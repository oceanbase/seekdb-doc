...
* 解決策：SQL文内のCASTステートメントが正しいか確認してください。例えば、暗黙的な変換が存在するかどうかを確認してください。
...
* エラー原因：EXECUTE時のバインド変数の数とPREPARE時のパラメータ数が一致しません。
...
* エラーの原因：`TIME_TO_USEC` 関数のパラメータは `VARCHAR`、`TIME`、`DATATIME`、または `TIMESTAMP` 型のみとして指定できます。
...
* エラー原因：`USEC_TO_TIME` 関数の引数は `TINYINT`、`MEDIUMINT`、`INT`、または `BIGINT` 型でなければなりません。
...
* エラー原因：権限を回収した際、その権限が付与されていないことが判明しました。
...
* エラー原因：以下のいずれかの状況が考えられます：
  ...
  * `CHR()` または `CSCONVERT()` の2番目のパラメータが `CHAR_CS` または `NCHAR_CS` でない場合。
...
* 解決策：読み取り専用のデータベースには操作を実行しないでください。
...
* 解決策：他のSQLが同じ `OUTLINE` とバインドされていないか確認してください。
...
* エラー原因：ゴミ箱内のオブジェクトには操作できません。

* 解決策：オブジェクトをゴミ箱から復元してから操作してください。

## ERROR 5270 (HY000) : object not in RECYCLE BIN

* seekdb エラーコード：5270

* エラー原因：オブジェクトはゴミ箱にありません。
...
* 解決策：レート制限を作成するステートメントに既存のレート制限ルールが含まれていないか確認してください。
...
* エラー原因：gapまたはoverlap時間帯を使用しています。
...
* エラー原因：外部キー列の長さは、参照先列の長さより短くてはなりません。
...
* エラー原因：公有シノニムの使用時に DATABASE を指定しました。

* 解決策：公有シノニムの使用時には DATABASE を指定する必要はありません。
...
* 解決策：PLで扱われるオブジェクトは既に存在しており、重複して作成することはできません。

## ERROR 5337 (HY000) : rowid is wrong

* seekdb エラーコード：5337

* エラー原因：`ROWID` から解析された内容が予想と一致しません。
...
* 解決策：連結後の長さが `group_concat_max_len` を超えないか確認します。超える場合は、`group_concat_max_len` を増やします。
...
* エラー原因：ユーザーのロールを取り消す際、そのロールがユーザーに付与されていないことが判明しました。
...
* エラー原因：`REVOKE` ステートメントでは、`CASCADE CONSTRAINTS` を指定する必要があります。
...
* エラー原因：自己自身に権限を付与または取り消すことはできません。

* 解決策：seekdbはユーザーが自己自身に権限を付与または取り消すことをサポートしていません。自身の権限を変更する必要がある場合は、システム管理者に連絡し、関連する操作を行ってもらってください。
...
* 解決策：テーブルの定義に変更があったかどうかを確認し、列の追加や削除、データ型や長さの変更などがないか確認します。
...
* seekdb エラーコード：5500
* エラー原因：生成列または関数インデックスの値に対する正当性チェックで、既存の値が現在の環境下で生成列またはインデックスの定義に基づいて再計算された値と異なることが検出されました。これは、システム変数の影響を受ける計算結果、または本質的に動作が不確定な組み込み関数を使用して生成列またはインデックスを定義したために発生する可能性があります。`_enable_defensive_check`構成パラメータが有効な場合、防御的にエラーが報告されます。
...
* エラー原因：現在のセッションで設定されたシステム変数の値が、関数インデックスまたは生成列を作成した際に固定された値と異なります。計算時には、関数インデックスまたは生成列は、最初に作成した際に固定された値を使用し、セッション内の現在の変数値ではありません。
...
* エラーの原因：不正なウィンドウ関数の使用。例：不適切なパラメータ設定など。

* 解決策：ウィンドウを修正し、正しいウィンドウ関数に変更してください。
...
* エラーの原因：分割前後で、パーティションの範囲が一致しません。
...
* エラーの原因：LISTパーティションにデフォルトパーティションが既に存在するため、新しいパーティションを追加できません。
...
* エラーの原因：使用中のテンポラリテーブルにインデックスの作成、変更、または削除を試みています。

* 解決策：テンポラリテーブルを使用しているすべてのセッション接続を切断してから、操作を実行してください。
...
* エラーの原因：時は1から12の間でなければなりません。

## ERROR 5645 (HY000) : hour must be between 0 and 23

* seekdb エラーコード：5645

* エラーの原因：時は0から23の間でなければなりません。
...
* 解決策：SQLステートメントの数字に無効な文字がないか確認してください。
...
* エラーの原因：`LIKE` クエリステートメントの `ESCAPE` フィールドの後に続く文字が無効です。
...
* エラーの原因：`LIKE` 計算時に、`ESCAPE` フィールドの後の文字が単一文字ではありません。

* 解決策：`ESCAPE` フィールドの後の文字を単一文字に指定してください。

## ERROR 5667 (HY000) : day of month must be between 1 and last day of month

* seekdb エラーコード：5667

* エラーの原因：月の日付は1からその月の最終日の間である必要があります。

## ERROR 5668 (HY000) : not a SELECTed expression

* seekdb エラーコード：5668

* エラーの原因：`GROUP BY` 句で式を指定しましたが、その式は `SELECT` 句に現れていません。
...
* seekdb エラーコード：5679
* MySQL互換エラーコード：4028
* エラー原因：関数インデックスはテーブルに隠れた列を追加します。テーブルに隠れた列がある場合、そのテーブルのすべての可視列(visible column)を削除するとこのエラーが返されます。
* 解決策：データテーブルの列定義とメタデータを確認し、削除操作後に少なくとも1列が可視列であることを確認します。
...
* エラー原因：DDLステートメントでは、列の可視性と他の列属性を同時に調整することはサポートされていません。
...
* エラー原因：内部エラーコード。PXにSQLの再試行を通知します。

* 解決策：通常このエラーは発生しません。このエラーが発生した場合は、システムのプライマリ切り替えやネットワーク状態が正常であるか確認してください。

## ERROR 5686 (HY000) : invalid extract field for extract source

* seekdb エラーコード：5686

* エラー原因：`EXTRACT` 関数が指定された値域を解釈できません。

* 解決策：指定された値域が有効か確認します。
...
* エラー原因：`CHECK`制約では `DATE` 関数やシステム変数の使用は許可されていません。
...
* エラー原因：`CHECK` 制約を満たしていません。

* 解決策：DMLで操作される列のデータが既存の `CHECK` 制約を満たしているか確認します。

## ERROR 5694 (HY000) : group function is not allowed here

* seekdb エラーコード：5694

* エラー原因：集約関数を不正な場所に記述しました。
...
* エラー原因：汎用的なラベルセキュリティエラーです。詳細は具体的なエラーメッセージを参照してください。

* 解決策：エラーメッセージに基づき、対応する操作を修正します。

## ERROR 5701 (HY000) : policy role already exists for policy string

* seekdb エラーコード：5701

* エラー原因：そのポリシー名は既に存在します。
...
* 解決策：インデックスが有効かどうか確認します。まず削除してから、再度作成することができます。
...
* エラー原因：Profile関連のDDLで、指定された `FAILED_LOGIN_ATTEMPTS`、`PASSWORD_LOCK_TIME` が合法的なINT型ではない、または値が `0` です。
...
* エラーの原因：Profileがユーザーに割り当てられているため、CASCADEを指定しないと削除できません。
...
* エラーの原因： `TZD`、`TZH`、`TZM`、`TZR` は `DATE` 形式に含めることはできません。
...
* エラーの原因：`INSERT`、`UPDATE`、`DELETE` ステートメントを使用して、更新できないビュー（生成列、式を変更する列、複数のテーブルからデータを取得する列など）を変更しようとしました。
...
* エラーの原因：削除対象のテーブルにkey-preserved tableが存在しない、または複数のkey-preserved tableが存在する、あるいはそのkey-preserved tableが読み取り専用ビューから取得したものである場合。

* 解決策：ビューを再定義するか、基礎テーブルに対して削除操作を実行してください。

## ERROR 5722 (HY000) : cannot modify a column which maps to a non key-preserved table

* seekdb エラーコード：5722

* エラー原因：JOINビュー内の非キープリーズドテーブルに対する挿入または更新を試みた場合。
...
* エラー原因：`INSERT`、`DELETE`、`UPDATE` ステートメントを使用して、読み取り専用として定義されたビューを変更しようとした場合。
...
* エラー原因：`PCTFREE` または `PCTUSED` パラメータ値が無効です。
...
* エラー原因：名前変更の対象となるテーブルオブジェクトには、ユーザー名を含めることはできません。
...
* 解決策：内部エラーコードです。お問い合わせフォームからお問い合わせいただき、サポートスタッフにご相談ください。

## ERROR 5763 (HY000) : Specified pseudo column or operator not allowed here

* seekdb エラーコード：5763

* エラー原因：この位置では `LEVEL`、`PRIOR`、`ROWNUM`、`CONNECT_BY_ROOT`、`CONNECT_BY_ISLEAF`、または `CONNECT_BY_ISCYCLE` キーワードの使用は許可されていません。
...
* 解決策：`SYS_CONNECT_BY_PATH` 関数に他の区切り文字を指定します。その区切り文字がどの列値にも含まれていないことを確認する必要があります。
...
* エラー原因：指定された `ORDER SIBLINGS BY` 句を含むクエリに `CONNECT BY` 句が含まれていません。
...
* エラー原因：クエリで `CONNECT_BY_ISCYCLE` が指定されていますが、クエリに `NOCYCLE` キーワードが含まれていません。
...
* エラーの原因：まだマージされていないビューから `FOR UPDATE` を選択しようとしました。ビューがマージされていないのは、ビューでマージを妨げる構文（例えば `DISTINCT` または `GROUP BY`）が使用されているか、初期化ファイルのパラメータ設定によるものです。
...
* エラーの原因：すでにそのポリシーで保護されているテーブルにポリシーを適用しようとしました。
...
* エラー原因：制約によって参照されている列を削除しようとした場合。
...
* エラー原因：制約によって参照されている親キー列を削除しようとした場合。
...
* エラー原因：`DISABLE VALIDATE` 制約を含むテーブルに対して、挿入、更新、または削除操作を試みた場合。
...
* エラー原因：自治PLブロックから戻る前に、そのブロック内で開始されたすべての自治トランザクションは完了（コミットまたはロールバック）していなければなりません。そうでない場合、実行中の自治トランザクションは暗黙的にロールバックされ、このエラーコードが返されます。

* 解決策：自治PLブロックから戻る前に、進行中のすべての自治トランザクションを明示的に指定またはロールバックすることを確認してください。

## ERROR 5797 (HY000) : ORDER BY not allowed here

* seekdb エラーコード：5797

* エラー原因：`DISTINCT` 関数と `RATIO_TO_REPORT` では `ORDER BY` を使用できません。
...
* エラー原因：`:=` を使用してユーザー変数に代入する場合、`SELECT` 句内でのみ使用可能であり、式のルートノードである必要があります。
...
* エラー原因：ネストされたトランザクション内で例外をキャッチすることは許可されていません。
...
* エラーの原因：`pivot` 操作内で集約関数以外の関数を使用しようとしました。
...
* 解決策：内部エラーコードです。サポートチームに連絡し、調査を依頼してください。
...
* エラーの原因：正規表現エンジンで異なる文字セットが混在している問題が発生しました。
...
* エラーの原因：正規表現の `pattern` で無効なマッチパラメータが指定されています。
...
* エラーの原因：正規表現エンジン内の状態遷移機械に異常が発生しました。
...
* エラーの原因：インデックスがVARRAY配列の最大長を超えているか、ネストされたテーブルに対してインデックスが大きすぎます。
...
* エラー原因：左辺でオブジェクト、`LOB`、またはその他の複合パーツに参照していますが、オブジェクト、`LOB`、またはその他の複合パーツが初期化されていません。
...
* エラー原因：ネストされたテーブルや `Varray` の要素、または `Member` 関数に参照していますが、コレクションが初期化されていません。
...
* エラー原因：パイプライン関数の呼び出し元は、パイプライン関数によってさらに行を生成する必要がありません。
...
* エラー原因：クエリ内の列数または列型が結果セット変数の宣言された戻り値型と一致しない、または2つの結果セット変数の宣言された型が一致しません。
...
* エラー原因：PL は他のストレージを割り当てることができません。このエラーは、制御不能なプログラムによって引き起こされることがあります。
...
* エラー原因：インデックスが `VARRAY` の制限値を超えているか、インデックスが `VARRAY` またはネストされたテーブルの非正の値です。
...
* 解決策：メソッド呼び出しのタイプを変更し、有効な `SELF` パラメータを渡します。
...
* エラー原因：テーブルの有効化制約を変更する操作が失敗しました。そのテーブルに含まれる一部の値が制約を満たしていないためです。

## ERROR 5852 (HY000) : Argument should be a constant

* seekdb エラーコード：5852

* エラー原因：パラメータは定数である必要があります。

## ERROR 5853 (HY000) : not a single-group group function

* seekdb エラーコード：5853

* エラー原因：単一グループではない `Group` 関数です。
...
* 解決策：seekdbドキュメントを参照し、そのパラメータの取り得る範囲を確認してください。
...
* エラー原因：`CHECK` 制約を持つ列のデータ型を変更しようとしました。`CHECK` 制約を持つ列では、データ型を `CHAR` から `VARCHAR` にのみ変更できます。
...
* エラー原因：`ALTER TABLE` または `ALTER INDEX` ステートメントが `RENAME` 操作を他の操作と結合しようとした場合、これは不正です。
...
* エラー原因：デフォルトのパーティション記述（`PARTITIONS number-of-partitions` によって生成される可能性があります）を要求する際に、`<(sub)partition-description>` 句を指定することは不正です。
...
* エラー原因：テーブルまたはインデックスの作成時に指定したサブパーティション名が一意ではありません。
...
* エラー原因：複数テーブルの `UPDATE` 時に `ORDER BY` を使用できません。
...
* エラー原因：複数テーブルの `UPDATE` 時に `LIMIT` を使用できません。
...
* 解決策：テクニカルサポートにお問い合わせいただき、調査のお手伝いをお願いします。
...
* エラー原因：入力日付にFXまたはFM形式コードを指定した場合、数値の桁数は形式コードで指定された桁数と完全に一致している必要があります。例えば、`9` は形式指示子DDにマッチしませんが、`09` はマッチします。
...
* エラー原因：非サブパーティションテーブルに対してサブパーティションのメンテナンス操作を実行しようとしています。
...
* エラー原因：存在しないプロシージャ、関数、パッケージ、またはパッケージ本体にアクセスしようとしています。
...
* 解決策：例外ハンドリングフェーズでエラースタックを確認し、具体的な例外を取得します。
...
* エラー原因：`CREATE TABLE` または `ALTER TABLE` ステートメント内の `REFERENCES` 句で指定された列リストに対応する、参照テーブルの一意キーまたは主キー制約が存在しません。
...
* エラー原因：テーブル作成時に、同一の外部キー制約を持つ参照列と参照される列が複数存在します。
...
* エラー原因：テーブル変更時に、同一の外部キー制約を持つ参照列と参照される列が複数存在します。
...
* エラー原因：生成列のデータ型は、現在サポートされていない `LOB` 型です。
...
* エラー原因：外部キーの参照列を指定しなかった場合、外部キーの主表に主キーがありません。
...
* エラー原因：拡張パーティションテーブル構文を使用してリモートオブジェクトに名前を付けようとしました。
...
* エラー原因：ビューに `EDITIONING_VIEW` を定義しようとしました。
...
* エラー原因：1つのテーブルに複数の識別列を指定しようとしました。

* 解決策：各テーブルには、識別列を1つのみ指定してください。
...
* エラー原因：識別属性ではない列の識別属性を変更しようとしました。

* 解決策：識別属性を持つ列の識別属性を変更してください。

## ERROR 5914 (HY000) : identity column cannot have a default value

* seekdb エラーコード：5914

* エラー原因：デフォルト値を識別列に追加しようとしました。
...
* エラー原因：識別列の値を非数値データ型に変更または作成しようとしました。
...
* エラー原因：識別列を持つプリビルドテーブルにインスタンス化ビューを作成しようとした場合。
...
* エラー原因：`GENERATED ALWAYS` キーワードを使用して作成された識別列に値を挿入しようとした場合。
...
* エラー原因：`GENERATED ALWAYS` キーワードを使用して作成された識別列を更新しようとした場合。
...
* エラー原因：`EXCHANGE` で指定された2つのテーブルの識別列のシーケンスが増加も減少もしない場合。
...
* エラー原因：システム生成のシーケンス名を変更しようとした場合。
...
* エラー原因：存在しないストアドプログラムを実行しようとした場合。
...
* エラー原因：新しいエンジンでは、再帰階層が深すぎるとエラーが発生します。この制限は 2 の 30 次方です。古いエンジンでは、ルートノードから現在のノードへのパス上で、すべてのノードとその子ノードの数が一定の値（2 の 30 次方）を超えるとエラーが発生します。現在の設定値は固定されており、通常はこの値を超える問題には遭遇しません。
...
* エラー原因：渡された引数値が `NULL`、無効、または範囲外です。例：`LOB / FILE` の位置またはサイズパラメータの値が1から(4GB-1)の範囲を超える場合、無効なオープンモードでファイルを開く場合など。
...
* エラー原因：無効なカーソルからアクセスを試みました。このエラーが発生する原因は以下の通りです：

  * 最後の行を取得して `OBE-1403` エラーを返した後、カーソルからアクセスを試みる。

  * `FOR UPDATE` 句を使用してカーソルを開いた後、`COMMIT` を実行してからアクセスを試みる。

  * SQLステートメント内のすべてのプレースホルダーを再バインドし、そのステートメントを再実行する前にアクセスを試みる。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>PLカーソルのループでは暗黙的に取得が行われるため、このエラーが発生する可能性もあります。</p>
  </main>
...
* エラー原因：リモート接続データベースでエラーが発生し、エラーコードをseekdbエラーコードにマッピングできません。

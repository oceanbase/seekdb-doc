...
* 解決策：XPath式とXMLデータが正しいか確認してください。
...
* 解決策：使用しているXPath式が正しいか、およびデータベースのバージョンがそのXPath式の使用をサポートしているか確認してください。
...
* 解決策：ユーザー '%.*s' がホスト '%.*s' 上で適切な権限を持ち、かつその権限が正しく定義されていることを確認してください。
...
* エラーの原因：現在のMySQLバージョンでは、JSON_TABLE式で列に非文字列型のデフォルト値を指定することはサポートされていません。
...
* エラーの原因：現在のバージョンはJSONモードでの参照の使用をサポートしていません。
...
## ERROR 1288 (HY000) : テーブル %.\*s の対象テーブル %.\*s は更新できません
...
## ERROR 1292 (22007) : %s=%d は %d と %d の間でなければなりません
...
## ERROR 1292 (01000) : 破損した不正確なDOUBLE値: '%.\*s'
...
## ERROR 1292 (22007) : 破損した不正確な %.\*s 値: '%.\*s'

* seekdb エラーコード：5222

* MySQL互換エラーコード：1292

## ERROR 1294 (HY000) : \\'%s\\' 列に有効なON UPDATE句がありません
...
## ERROR 1298 (HY000) : 未知または誤ったタイムゾーン: \\'%.\*s\\'

* seekdb エラーコード：5192

* MySQL互換エラーコード：1298

## ERROR 1300 (HY000) : 有効でない %.\*s 文字列: \\'%.\*s\\'
...
* エラーの原因：ユーザーに対してルーチンコマンドの実行を拒否しました。

* 解決策：ユーザーが対応する権限を持っているか確認し、ストアドプロシージャまたは関数にスペルミスがないか確認します。
...
* エラーの原因：プリペアドステートメントが自身への再帰呼び出しを含んでいます。
...
* エラーの原因：PARTITION BY句でVALUES LESS THANを使用できるのはRANGE PARTITIONINGのみです。
...
* 解決策：
  * パーティショニング方式の変更：RANGE/LIST パーティションと HASH/KEY パーティションを同時にサブパーティションで使用したい場合は、互換性のあるパーティショニング方式に変更することを検討してください。パーティショニング方式の変更やパーティションテーブルの分割は、既存のデータやアプリケーションに影響を与える可能性があるため、慎重に操作してください。
...
* seekdb エラーコード：5493
* MySQL互換エラーコード：1517
* エラー原因：テーブルのパーティション名を変更する際、そのパーティション名が既存のテーブルのパーティション名と重複していました（MySQL自体にサブパーティションの概念がないため、パーティションとサブパーティションの名前はこのエラーコードを共有します）。
...
* 解決策：
  ...
  * プラグインをロードおよびインストールするための十分な権限があるか確認します。
...
* エラー原因：作成しようとするイベント名が既存のイベント名と競合しています。
...
* エラーの原因：アクセスまたは操作を試みたイベントが存在しません。

* 解決策：イベント名が正しいか確認し、対象のイベントが存在するか確認してください。
...
* 解決策：`ALTER EVENT` ステートメントの構文が正しいことを確認してください。これには、変更するプロパティと値が含まれます。
...
* エラーの原因：イベント、テーブル、またはその他のデータベースオブジェクトの削除（DROP）に失敗しました。

* 解決策：対応するクエリステートメントを使用して、オブジェクトの正確な名前と存在を確認してください。
...
* エラーの原因：イベント(Event)を定義する際に指定した時間間隔(INTERVAL)が正の数でないか、許容される最大値を超えています。

* 解決策：INTERVALの値が正の数であることを確認してください。間隔値が大きすぎる場合は、要件とシステムの制限に基づいて適切な範囲に調整してください。
...
* エラーの原因：イベントの終了時刻(ENDS)が無効であるか、開始時刻(STARTS)より前です。
...
* エラーの原因：イベント作成時に設定された開始時刻が現在時刻より早いです。
...
* エラーの原因：`mysql.event` テーブルからイベントの削除に失敗しました。

* 解決策：現在のユーザーにイベント削除権限があることを確認してください。
...
* エラーの原因：ある列に挿入したデータの長さが、その列で定義された最大長を超えています。
...
* エラー原因：DDL操作が、自身を直接または間接的に再変更しようとしています。イベント（EVENT）定義に本体（body）が含まれる場合、イベントのDDLステートメントの再帰実行は禁止されています。
...
* エラー原因：変更を試みたイベントの実行時間が過去にあたるため、変更は反映されませんでした。

* 解決策：変更後のイベントの実行時間が未来になるようにして、イベントが正常にスケジュールされるようにしてください。
...
* エラー原因：交換を試みているテーブルがパーティションテーブルです。
...
* 解決策：ロール '%.*s' が正しく付与され、システム内に存在することを確認してください。
...
* エラーの原因：ユーザー変数名が無効です。長さは1024文字を超えてはなりません。

* 解決策：ユーザー変数名を有効な名前に変更してください。
...
* 解決策：クエリステートメントを確認し、変数やパラメータが含まれていないことを保証してください。
...
* エラーの原因：認証ID '%.*s'@'%.*s' は既に存在します。
* 解決策：異なる認証IDを使用するか、既存の認証IDを削除してから、再度認証IDの作成を試みてください。
...
* エラー原因：RANGEフレームを持つウィンドウには、日付時刻型に基づくORDER BY式が含まれており、INTERVAL境界値のみの使用が許可されています。

* 解決策：RANGEフレームを持つウィンドウのORDER BY式には、日付時刻型の境界値を使用せず、INTERVAL境界値のみを使用するようにします。
...
* エラー原因：RANGEフレームを持つウィンドウには、数値型のORDER BY式が含まれており、INTERVAL境界値の使用は許可されていません。
...
* エラー原因：ウィンドウには定数ではないフレーム境界があります。
...
* 解決策：配列またはオブジェクトをスカラー値に変換するか、配列またはオブジェクトの格納に適したデータ型にカラムのデータ型を変更します。
...
* seekdb エラーコード：5496
* MySQL互換エラーコード：3754
* エラー原因：関数インデックスの作成時に、自動増分列を関数インデックスの一部として使用しましたが、自動増分列は関数インデックスの一部として使用できません。
...
* seekdb エラーコード：5498
* MySQL互換エラーコード：3757
* エラー原因：関数インデックスの作成時に、BLOBまたはTEXT型を返す式を使用しましたが、BLOBまたはTEXT型の式は関数インデックスの作成に使用できません。
...
* seekdb エラーコード：5905
* MySQL互換エラーコード：3758
* エラー原因：関数インデックスの作成時に、サポートされていない関数を使用しました。インデックス関数にSYSDATEやユーザー環境が使用されています。
...
* seekdb エラーコード：5499
* MySQL互換エラーコード：3762
* エラー原因：関数インデックスの作成時に、インデックス関数が単一列でした。
...
* エラー原因：パラメータに有効でないJSONタイプが使用されています。実際にはオブジェクトタイプです。
...
* seekdb エラーコード：5502
* MySQL互換エラーコード：3855
* エラー原因：列にパーティション関数の依存関係があるため、その列を削除または名前変更できません。
...
* エラーの原因：INSERTステートメント内でデータソースとして使用される場合を除き、VALUES句内の各行には少なくとも1つの列が必要です。
...
* エラーの原因：INSERTステートメント内でデータソースとして使用される場合を除き、VALUES句ではデフォルト値を使用できません。

* 解決策：INSERTステートメント内でデータソースとして使用される場合を除き、VALUES句でデフォルト値を使用しないでください。

...
* 解決策：正しい資格情報を再入力してください。ユーザー名とパスワードが正しいことを確認し、入力ミスがないことを確認してください。
...
* 解決策：操作を実行する際、指定した行キーの列数がテーブルの行キーの列数と一致することを確認してください。
...
* エラー原因：OBKV [checkAndInsertup インターフェース](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002025908)のrollbackWhenCheckFailedパラメータが `true` に設定されている場合、チェックに失敗するとこのエラーコードがスローされます。

* 解決策：このエラーがスローされた場合、チェックに失敗したことを意味します。業務の実際の状況に応じて、無視するか他の処理を行ってください。
...
* エラー原因：`PARSE_HEADER` と `SKIP_HEADER` を同時に使用することはできません。意味的に矛盾します。

* 解決策：外部ファイル形式に関連するプロパティを指定する際に、`PARSE_HEADER` と `SKIP_HEADER` パラメータを同時に使用しないでください。

## ERROR 11082 (HY000) : DDL operations on the internal catalog are not allowed

* seekdb エラーコード：11082

* エラー原因：内部カタログ(Internal Catalog)に対するDDL操作は許可されていません。

* 解決策：操作対象のCatalogのタイプを確認してください。

## ERROR 11083 (HY000) : Command denied to user '%.*s'@'%.*s' for catalog '%.*s'

* seekdb エラーコード：11083

* エラー原因：ユーザーにはCatalogの操作権限がありません。

* 解決策：ユーザーがCatalogの権限を持っているかどうかを確認し、該当するCatalogの操作権限をユーザーに付与してください。

## ERROR 11088 (HY000) : reject limit %ld reached

* seekdb エラーコード：11088

* エラー原因：拒否制限 %ld に達しました。

* 解決策：診断エラー制限情報を確認してください。

## ERROR 11094 (HY000) : Sensitive rule '%.*s' exists

* seekdb エラーコード：11094

* エラー原因：同名の既存の機密ルールが存在します。通常、DDLステートメントで機密ルールを新規作成する際、新規ルールと既存ルールが名前が重複します。

* 解決策：既存の機密ルールを削除してから、再試行してください。

## ERROR 11095 (HY000) : Sensitive rule '%.*s' does not exist

* seekdb エラーコード：11095

* エラー原因：指定された名前の機密ルールは存在しません。通常、DDLステートメントで機密ルールを削除する際、存在しないルール名を指定します。

* 解決策：機密ルールが存在するかどうかを確認してから、再試行してください。

## ERROR 11096 (HY000) : Sensitive column %.*s(%.*s) exists

* seekdb エラーコード：11096

* エラー原因：指定された名前の列は既に機密列です。通常、DDLステートメントで機密ルールを新規作成または変更する際、他の機密ルールでカバーされている列に機密ルールを適用しようとします。

* 解決策：その列が他の機密ルールでカバーされていないかどうかを確認してください。もしそうであれば、既存の機密ルールを削除するか、他の未カバーの列を選択して新しい機密ルールを適用する必要があります。

## ERROR 11097 (HY000) : Sensitive column %.*s(%.*s) does not exist

* seekdb エラーコード：11097

* エラー原因：指定された名前の機密列は存在しません。通常、DDLステートメントで機密ルールを変更する際、機密列ではない列を削除しようとします。

* 解決策：削除する列が実際に機密列であるかどうかを確認し、機密ルールの設定を確認して、指定された列名とテーブル名が正しいことを確認してください。

## ERROR 11098 (HY000) : Access sensitive field protected by sensitive rule '%.*s' denied to user '%.*s'@'%.*s

* seekdb エラーコード：11098

* エラー原因：対応する機密ルールへのアクセス権限がありません。

* 解決策：データベース管理者に連絡し、現在のユーザーにその機密ルールへのアクセス権限を付与してもらうか、対応する権限を持つユーザーアカウントを使用して操作してください。
...
* 解決策：機密ルール名が命名規則に準拠しているかどうかを確認し、名前に特殊文字が含まれていないこと、長さが許容範囲内であること、既存のルールと名前が重複していないことを確認してください。
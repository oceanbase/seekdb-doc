...
| tabname       | テーブル名。                                                                      |
| partname      | テーブルのパーティション名です。テーブルがパーティション化されており、かつ `partname` が `NULL` の場合は、グローバルおよびパーティションの列統計情報をインポートします。    |
...
|    エラーコード    |        説明         |
|-----------|-------------------|
| HY000     |  <ul><li>オブジェクトが存在しないか、権限が不足しています。</li><li>ユーザー情報統計テーブルの値が無効または一貫性がありません。</li></ul>     |

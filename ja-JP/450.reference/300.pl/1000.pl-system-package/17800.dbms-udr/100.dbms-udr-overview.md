...
構成パラメータ `enable_user_defined_rewrite_rules` でリライトバインディング機能の有効化を制御し、ルール作成時に指定するデータベースによって権限を管理します。

## DBMS_UDRサブプログラムの概要

次の表は、seekdbがサポートする `DBMS_UDR` サブプログラムとその簡潔な説明をリストアップしたものです。

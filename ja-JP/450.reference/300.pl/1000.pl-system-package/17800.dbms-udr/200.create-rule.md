...
| replacement | `pattern` に一致するステートメントを書き換える方法を指示するテンプレート。 |
| enabled | ルールが有効かどうか。 |
...
- `rule_name` はseekdb内で一意である必要があります。
- ルールテンプレート内の `pattern` と `replacement` のパラメータマーカーの数は同じでなければなりません。
- ルールテンプレート内の `pattern` と `replacement` のパラメータ表現方法は同じでなければなりません（どちらも「?」で表すか、どちらも「:`{name}`」で表すかのどちらかです）。

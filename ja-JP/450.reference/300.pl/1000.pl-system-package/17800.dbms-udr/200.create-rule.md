...
- 定義されたルールテンプレート内で `?` と `:{name}` を混在して使用することはできません。
- `pattern` の定義で `{:name}` をパラメータマーカーとして使用する場合、`name` は重複してはなりません。
- `pattern` と `replacement` を空に定義することはできません。
- `pattern` と `replacement` はマルチクエリをサポートしません。
- `rule_name` は seekdb 内で一意である必要があります。
- ルールテンプレート内の `pattern` と `replacement` のパラメータマーカーの数は同じでなければなりません。
- ルールテンプレート内の `pattern` と `replacement` のパラメータ表現方法は同じでなければなりません（どちらも「?」で表すか、またはどちらも「:`{name}`」で表す）。

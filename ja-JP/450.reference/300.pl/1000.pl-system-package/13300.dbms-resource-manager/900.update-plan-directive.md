...
| NEW_UTILIZATION_LIMIT | CPUリソース使用比率の上限です。このパラメータの取り得る範囲は \[0, 100\] です。「100」はseekdbの全CPUリソースを最大限に使用できることを意味します。値が「20」の場合は、seekdbの20%のCPUリソースを最大限に使用できることを意味します。 |
...
リソースプランの指示に含まれる数値パラメータをクリア（ゼロまたは無効に）するには、`UPDATE_PLAN_DIRECTIVE` プロシージャを使用してそれらを -1 に設定します。

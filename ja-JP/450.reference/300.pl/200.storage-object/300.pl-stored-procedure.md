...
ストアドプロシージャは、直接的な戻り値を持たないサブルーチンです。パラメータの型が `OUT` の場合、ストアドプロシージャは呼び出し元に値を返すこともできます。
...
* `DECLARE` ブロックは `BEGIN ... END;` ブロック内に配置する必要があり、すべての定義と宣言が完了してから他のステートメントを定義できます。

* `DETERMINISTIC / LANGUAGE SQL` などの `sp_create_chistic` 情報を定義することで、ストアドプロシージャの使用を拡張できます。

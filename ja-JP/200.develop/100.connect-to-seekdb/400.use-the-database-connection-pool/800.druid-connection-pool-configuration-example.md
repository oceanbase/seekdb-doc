...
2. seekdb接続文字列の情報に基づいて、以下のURLの該当する情報を入力します。
...
1.  データベース接続パラメータの設定
    ...
    *   `$host`：seekdbへの接続IPアドレスを提供します。実際のIPアドレスに置き換えて使用するか、ローカルIPアドレスや127.0.0.1を使用できます。
    *   `$port`：seekdbの接続ポートを提供します。実際のポート番号に置き換えて使用してください。デフォルトは2881ですが、seekdbのデプロイ時にカスタマイズ可能です。
...
2.  その他の接続プールパラメータの設定
    ...
    4.  廃棄接続のログを出力するかどうかを `true` で指定します。これは、廃棄接続が回収される際にエラーログに情報が出力されることを意味します。テスト環境では `true` に設定し、本番環境ではパフォーマンスへの影響を避けるために `false` に設定できます。
    ...
    8.  廃棄接続を回収するかどうかを `true` で指定します。これは、接続が `removeAbandonedTimeout` で定義された時間を超えると回収されることを意味します。
    9.  廃棄接続のタイムアウト時間を300秒に設定します。これは、300秒（5分）以上使用されない接続が回収されることを意味します。
    ...
    15.  接続のアイドル時間しきい値を60000ミリ秒に設定します。これは、接続のアイドル時間が設定されたしきい値60000ミリ秒（1分）を超えると、接続の可用性を確保するために接続ハイビジビメカニズムがその接続を検出することを意味します。接続がしきい値時間内に何らかの操作を行うと、接続のアイドル時間は再計算されます。
...
| initialSize | 接続プールの初期化時に作成する接続数を指定します。アプリケーション起動時、接続プールは指定された数の接続を作成してプールに格納します。|
...
| testWhileIdle | アイドル状態のときに接続をテストするかどうかを指定します。`true` に設定すると、接続がアイドル状態のときに `validationQuery` を実行し、接続が有効かどうか検証します。|
| testOnBorrow | 接続を取得する際に接続をテストするかどうかを指定します。`true` に設定すると、接続を取得する際に `validationQuery` を実行し、接続が有効かどうか検証します。|
| testOnReturn | 接続を返却する際に接続をテストするかどうかを指定します。`true` に設定すると、接続を返却する際に `validationQuery` を実行し、接続が有効かどうか検証します。|
| filters | 接続プール内で事前定義された一連のフィルターを指定します。これらのフィルターは、特定の順序で接続に対する事前処理および後処理操作を行い、追加機能を提供し、接続プールのパフォーマンスを向上させることができます。一般的なフィルターには以下のものがあります：<ol><li>`stat`：接続プールのパフォーマンス指標を統計するために使用されます。例えば、アクティブ接続数、リクエスト数、エラー数などです。</li><li>`wall`：SQLファイアウォールとして使用され、安全でないSQLステートメントを遮断・無効化してデータベースのセキュリティを向上させます。</li><li>`log4j`：接続プールのログを `log4j` に出力するために使用され、ログ記録とデバッグを容易にします。</li><li>`slf4j`：接続プールのログを `slf4j` に出力するために使用され、ログ記録とデバッグを容易にします。</li><li>`config`：外部設定ファイルから接続プールの設定情報を読み込むために使用されます。</li><li>`encoding`：接続プールとデータベース間の文字エンコーディングを設定するために使用されます。</li></ol>これらのフィルターを `filters` プロパティに設定することで、接続プールは指定された順序で順次これらのフィルターを適用します。複数のフィルター名はカンマで区切ることができます。例：`filters=stat,wall,log4j`。|
...
2. `Main` クラスを作成し、`main` メソッドを定義します。
    ...
    6. `try-with-resources` ステートメントを使用して、データベース接続を取得し、接続の使用後に自動的に接続を閉じます。
        ...
        5. 更新されたデータを再クエリするために、`selectData()` メソッドを再度呼び出します。
        6. データを削除するために、`deleteData()` メソッドを呼び出します。
        7. 削除後のデータを再クエリするために、`selectData()` メソッドを再度呼び出します。
...
3. プロパティファイル内の設定情報を取得し、使用するためのメソッドを定義します。
    ...
    1. プロパティファイルを読み込んで `Properties` オブジェクトを返すプライベート静的メソッド `loadPropertiesFile()` を定義し、`IOException` 例外がスローされる可能性があることを宣言します。
...
4. データソースオブジェクトを取得するためのメソッドを定義します。
    ...
    1. プロパティファイルの `Properties` オブジェクトをパラメータとして受け取り、`Exception` 例外がスローされる可能性があることを宣言するプライベート静的メソッド `createDataSource()` を定義します。
...
5. テーブルを作成するためのメソッドを定義します。
    ...
    1. データベースに `Connection` オブジェクトをパラメータとして受け取り、`SQLException` 例外がスローされる可能性があることを宣言するプライベート静的メソッド `createTable()` を定義します。
...
6. データを挿入するためのメソッドを定義します。
    ...
    1. データベースに `Connection` オブジェクトをパラメータとして受け取り、`SQLException` 例外がスローされる可能性があることを宣言するプライベート静的メソッド `insertData()` を定義します。
    2. データ挿入用のSQLステートメントを格納する文字列変数 `insertDataSql` を定義します。
    3. データ挿入行数を記録するための整数変数 `insertedRows` を定義し、初期値を0に設定します。
    ...
    5. `for` ループステートメントを使用して、5回繰り返し、5件のデータを挿入します。
    ...
    6. データ挿入成功のメッセージと、挿入した総行数を出力します。
    7. 挿入した総行数を返します。
...
7. データを更新するためのメソッドを定義します。
    ...
    5. `executeUpdate()` メソッドを使用してデータ更新用のSQLステートメントを実行し、戻り値としての操作行数を `updatedRows` 変数に代入します。
    6. データ更新成功のメッセージと、更新した総行数を出力します。
...
8. データを削除するためのメソッドを定義します。
    ...
    4. `executeUpdate()` メソッドを使用してデータ削除用のSQLステートメントを実行し、戻り値としての操作行数を `deletedRows` 変数に代入します。
    5. データ削除成功のメッセージと、削除した総行数を出力します。
...
9. データをクエリするためのメソッドを定義します。
    ...
    4. データクエリ用のSQLステートメントを実行するために `executeQuery()` メソッドを使用し、戻り値としての結果セットを `resultSet` 変数に代入します。

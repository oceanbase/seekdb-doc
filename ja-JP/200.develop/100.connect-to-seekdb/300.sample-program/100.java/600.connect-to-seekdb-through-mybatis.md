...
# MyBatis による seekdb 接続サンプルプログラム

本記事では、MyBatisフレームワークとseekdbを使用して、テーブルの作成、データの挿入、クエリなどの基本的な操作を実現するアプリケーションの構築方法を紹介します。
...
2. デプロイ済みのseekdbに基づいて、以下のURLの該当情報を入力します。
    ...
    * `port`：seekdbのポート番号。実際のポートに置き換えてください。デフォルトは2881ですが、seekdbのデプロイ時にカスタマイズ可能です。
...
* `IUserMapper.xml`：ユーザーデータアクセス層のXML設定ファイル。
...
コード内の各フィールドの値は、**ステップ1：seekdb接続文字列を取得する**から直接取得します。

:::

* `jdbc.driver`：データベースドライバーのクラス名で、データベースドライバーのロードに使用されます。
...
3.  MyBatisのグローバル設定を構成します。
    ...
    *   `mapUnderscoreToCamelCase`：データベース列名のアンダースコアをJavaオブジェクトのプロパティ名におけるキャメルケースに変換するかどうかを構成するために使用されます。`true` に設定します。
...
`IUserMapper.xml` ファイルはマッピングファイルであり、ユーザーオブジェクトに関連するSQLステートメントを定義するために使用されます。このファイルでは、ユーザーオブジェクトに関連するSQLステートメントを格納するための `IUserMapper` という名前空間が定義されています。
...
2. `IAppMapper` インターフェースの定義。
   `IAppMapper` インターフェースは、MyBatisの `Mapper` インターフェースを定義するために使用されます。`Mapper` インターフェースはSQLマッピング関係を定義し、データベースに対する追加、削除、変更、検索操作を実装します。具体的には、`IAppMapper` インターフェースは `test_app` テーブルに対する追加、削除、変更、検索のメソッドを定義します。これには以下が含まれます：

   * `@Insert` メソッド：データベースにデータを挿入するために使用されます。
   * `@Update` メソッド：データベース内のデータを更新するために使用されます。
   * `@Delete` メソッド：データベースからデータを削除するために使用されます。
...
3. `testSqlSession` メソッドの定義。
    ...
    11. `statement.close()` を使用して、データベース接続内の `Statement` オブジェクトを閉じます。次に、`sqlSession.commit()` を使用してトランザクションをコミットし、すべての変更をデータベースに永続化します。最後に、`sqlSession.close()` を使用して `SqlSession` オブジェクトを閉じ、関連するリソースを解放し、データベース接続を閉じます。

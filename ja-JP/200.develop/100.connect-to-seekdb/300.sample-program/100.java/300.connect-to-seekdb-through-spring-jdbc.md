...
1. seekdbのデプロイ担当者または管理者から、該当するデータベース接続文字列を取得します。
    ...
    **例：**
    ...
    接続文字列の詳細については、[MySQLクライアントからseekdbへの接続](../../200.use-client-connect-seekdb/100.connect-to-seekdb-by-using-a-mysql-client.md)を参照してください。
...
1.  パッケージの定義と必要なクラスのインポート。
    ...
    2.  `JdbcTemplate` クラスをインポートします。これはSpring JDBCフレームワークのコアクラスであり、データベース操作の実行に使用されます。
    3.  `DriverManagerDataSource` クラスをインポートします。これはSpringが提供する `DataSource` インターフェースを実装したデータソースクラスであり、データベース接続情報の設定に使用されます。
...
3.  データベース接続情報を定義します。
    ...
    *   `$host`：seekdbへの接続IPアドレス。実際のIPアドレスに置き換えてください。ローカルIPアドレスや127.0.0.1を使用することもできます。
    *   `$port`：seekdbの接続ポート。実際のポート番号に置き換えてください。デフォルトは2881ですが、seekdbのデプロイ時にカスタマイズ可能です。
...
4.  データソースを作成します。
    ...
    `DriverManagerDataSource` オブジェクトを作成し、ドライバークラス名、データベースURL、ユーザー名、パスワードを含むデータベース接続に関する情報を設定します。具体的な手順は以下の通りです：
...
6.  テーブルを作成します。
    ...
    1.  テーブル作成用のSQLステートメントを定義します。SQLステートメントでは、テーブル名を `test_springjdbc` と指定し、`id` という名前の `INT` 型列と `name` という名前の `VARCHAR(20)` 型列の2つの列を定義します。
...
8.  データを更新します。
    ...
    1.  データベーステーブル `test_springjdbc` 内の `id` が `1` のレコードの `name` フィールドの値を `A11` に更新するSQLステートメントを定義します。
...
9.  データを削除します。
    ...
    1.  データベーステーブル `test_springjdbc` から `id` が `2` のレコードを削除するSQLステートメントを定義します。
...
11. テーブルを削除します。
    ...
    1.  データベースからテーブル `test_springjdbc` を削除するSQLステートメントを定義します。

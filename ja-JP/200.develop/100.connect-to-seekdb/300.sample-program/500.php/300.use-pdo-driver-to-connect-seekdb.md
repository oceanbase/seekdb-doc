...
# PDOドライバーを使用してseekdbに接続する

本記事では、PDOドライバーを使用してseekdbに接続し、テーブルの作成やデータの挿入などの基本的な操作を実行する方法について説明します。
...
### ステップ２：podtest.phpファイル内のデータベース接続情報を修正する
...
* `$username` はseekdbユーザー名を指します。
* `$password` はseekdbへの接続パスワードを指します。
* `$dbname` は接続するデータベース名を指します。
...
テーブルの作成および追加・削除・更新・クエリ操作のテスト例は以下のとおりです。

```shell
[root]# vim podtest.php
<?php
$servername = "172.30.xxx.xxx"; // seekdb サーバーアドレス
$port = "2881"; // seekdb サーバーのポート番号
$username = "root"; // seekdb ユーザー名
$password = ""; // seekdb 接続パスワード

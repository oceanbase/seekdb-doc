...
# Sequelとseekdbを使用した接続サンプルプログラム

本記事では、Sequelとseekdbを使用してアプリケーションを構築し、テーブルの作成、データの挿入、クエリなどの基本的な操作を実現する方法について説明します。
...
* `$host`：seekdb接続用IPアドレスを提供する
* `$port`：seekdb接続ポートを提供する
* `$database_name`：アクセスするデータベース名
* `$user_name`：seekdbの接続アカウントを提供する

...
AI MODELに関する権限は、AIモデルを管理するために使用され、`CREATE AI MODEL`、`ALTER AI MODEL`、`DROP AI MODEL` の3つの権限が含まれます。このドキュメントでは、AI MODELに関する権限について説明します。

## AI関数呼び出し権限との関係

`AI MODEL` 関連権限は、AIモデルおよびエンドポイントの登録と管理に使用されます。一方、`ACCESS AI MODEL` 関連権限はSQLでAI関数を呼び出すために使用されます。モデルやエンドポイントの登録・管理を行う際には `AI MODEL` 権限のみが必要であり、`ACCESS AI MODEL` 権限は不要です。しかし、SQLでAI関数を正常に呼び出すには、モデルとエンドポイントの登録（`AI MODEL` 権限が必要）が完了しているだけでなく、`ACCESS AI MODEL` 呼び出し権限も必要です。つまり、**モデル登録とAI関数の呼び出しはそれぞれ異なる権限に対応しており、実際のシナリオでは通常両方の権限が同時に必要となります**。

...
* 既にseekdbに接続していること。
* `SELECT`権限を保有していること。現在のユーザー権限を確認するその他の操作については、[ユーザー権限の確認](../../../400.guides/500.manage/300.security/300.access-control/200.user-and-permission/400.view-user-permissions.md)を参照してください。この権限がない場合は、管理者に連絡し権限付与を依頼してください。ユーザー権限に関する操作については、[直接権限付与](../../../400.guides/500.manage/300.security/300.access-control/200.user-and-permission/200.direct-authority.md)を参照してください。

## 構文

[SELECT](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002017573) ステートメントを使用してデータのクエリを実行してください。

`SELECT` ステートメントによる単一テーブルのクエリの一般的な構造は次のとおりです：
...
| ASC | DESC                  | オプションパラメータ。ソート順序を指定します。ASCは昇順（デフォルト）、DESCは降順を表します。|
| LIMIT limit_clause          | オプションパラメータ。返される結果セットの行数を制限します。|
| column_list                 | 検索する列を指定するパラメータです。単一の列または複数の列をカンマで区切って指定できます。|
| column_name                 | 検索する列名。|

### SELECTキーワードの実行順序の概要

`WHERE`、`GROUP BY`、`HAVING`、`ORDER BY`、`LIMIT` これらのキーワードを一緒に使用する場合、実行の順序には明確な制限があります。キーワードの実行順序は以下のとおりです：

1. `FROM` を実行してテーブルを特定します。

2. `WHERE` を実行して制約条件を指定します。

3. `GROUP BY` を実行して、取得した各レコードをグループ化（集計）します。`GROUP BY` がない場合は、全体を1つのグループとします。
...
`WHERE` と `HAVING` の違いは、`WHERE` がグループ化の前にデータをフィルタリングするのに対し、`HAVING` はグループ化後の結果でフィルタリングを行い、最終的にSQL全体のクエリ結果を返すことです。
:::
...
`SELECT` を使用する際は、意味のある列アルイアスと適切な列の順序を使用することを推奨します。これにより、結果セットの可読性が向上し、クエリ結果をより効果的に整理し理解できます。

### すべての列を照会する

* `SELECT * FROM student;` ステートメントを使用して、すべての学生情報を照会できます。

* すべてのフィールドを手動で列挙し、`SELECT id,name,gender,age,score,enrollment_date FROM student;` ステートメントを使用して、すべての学生情報を照会することもできます。
...
等しい（`=`）：指定された列の値が特定の値と等しいデータを検索します。値が文字列タイプの場合は、シングルクォートまたはダブルクォートで囲む必要があります。

例4：以下のSQLステートメントを使用して、テーブル `student` 内の列 `gender` が `1` に等しいすべての行を検索し、それらの行の `id`、`name`、`gender` 列のデータを返します。
...
例5：以下のSQLステートメントを使用して、テーブル `student` 内の列 `gender` が `1` と等しくないすべての行を検索し、それらの行の `id`、`name`、`gender` 列のデータを返します。
...
大なり（`>`）と小なり（`<`）は、数値の大小を比較します。文字を比較する場合は、ASCIIコードに対応する値に基づいて比較し、文字が対応する位置から順に比較します。

:::info

大なりイコール（\>=）と小なりイコール（\<=）も同様です。

:::

例6：以下のSQLステートメントを使用して、テーブル `student` 内の列 `score` が `90` より小さいすべての行を検索し、それらの行の `id`、`name`、`score` 列のデータを返します。
...
論理クエリ演算子であるAND（`AND`）とOR（`OR`）は、複数の条件を組み合わせたクエリをサポートします。
...
例8：次のSQLステートメントを使用して、テーブル `student` 内で `gender` が `1` に等しいか `score` が `90` 未満のすべての行を検索し、これらの行の `id`、`name`、`gender`、`score` 列のデータを返します。
...
`LIKE` 述語は、文字列のあいまい一致を行うために使用できます。
...
比較演算子、`LIKE`、`BETWEEN AND`、`IN`、`NOT IN` は `NULL` 値に対して正確なクエリ結果を返さないため、`NULL` 値専用のクエリステートメントである `IS NULL` および `IS NOT NULL` の使用を推奨します。その他にも、通常の数値と `NULL` 値の両方を判断できる安全な等価演算子（`<=>`）を使用することができます。
...
SQLクエリでは、`GROUP BY` 句を使用してクエリ結果をグループ化できます。`GROUP BY` は単一フィールドのグループ化と複数フィールドのグループ化をサポートします。グループ化の前には `WHERE` 句でデータをフィルタリングし、グループ化後には `HAVING` 句でデータをフィルタリングし、`ORDER BY` 句でデータをソートすることもできます。
...
データのソートとは、指定された列または式に基づいてクエリ結果を並べ替える操作です。昇順（`ASC`）または降順（`DESC`）でデータを並べ替えることができます。SQLクエリでは、`ORDER BY` 句を使用してソート方法を指定します。`ORDER BY` 句は、単一フィールドのソート、複数フィールドのソート、エイリアスによるソート、関数によるソートをサポートしており、複数フィールドのソートではカンマで区切ります。ソートクエリを実行する際に `ASC` または `DESC` キーワードを指定しない場合、クエリ結果はデフォルトで昇順に並べ替えられます。
...
`ORDER BY` 句では、関数を使用してクエリ結果をソートできます。関数はソート対象の列または式に適用することで、データに対してより複雑なソートを実現できます。
...
例27：`student` テーブルで、1ページに2件のデータを表示し、1ページ目、2ページ目、3ページ目のデータを順次取得します。

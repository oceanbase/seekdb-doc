...
集計関数は、一連のレコードをスキャンして、単一行の結果を返します。この一連のレコードは、テーブルまたはビュー、あるいはサブクエリの結果です。
...
seekdbでは、集約関数を `GROUP BY` 句と共に使用する場合、`select_list` 内の列に特別な要件はありません。`select_list` の列が `GROUP BY` 句の列と一致する必要がある場合は、MySQLコマンドラインの `sql_mode` を `ONLY_FULL_GROUP_BY` に設定する必要があります。例：

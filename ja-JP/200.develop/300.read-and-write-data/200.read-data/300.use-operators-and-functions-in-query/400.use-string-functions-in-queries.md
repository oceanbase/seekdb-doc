...
よく使われる文字列関数には、文字列の長さを取得するLENGTH()とCHAR_LENGTH()、文字列を切り出すSUBSTR()、文字列を結合するCONCAT()とCONCAT_WS()、大文字と小文字を変換するUPPER()とLOWER()、文字列の前後接辞を削除するLTRIM()、RTRIM()、TRIM()などがあります。
...
文字列関数 CHAR_LENGTH()、CONCAT()、CONCAT_WS()、SUBSTR()、UPPER() を使用してデータをクエリします。
...
注意点として、seekdbでは、文字列長関数 `LENGTH()` の長さの単位はバイト、`CHAR_LENGTH()` 関数の文字列長の単位は文字です。例：

...
本記事では、具体的な例を通じてクエリ内で算術演算子を使用する方法を説明します。

## 算術演算子

クエリで一般的に使用される算術演算子には、+（加）、-（減）、\*（乗）、/（除）、-（符号反転）、MOD（剰余）があります。これらの演算子は数値データ型の列に対して使用できます。
...
条件に合致する商品の数量と価格を照会し、数量に価格を掛けることで各商品の支払総額が得られます。`select_list` に列 `t2.ol_quantity * t3.i_price` を追加し、`item_sum_price` という名前に変更します。

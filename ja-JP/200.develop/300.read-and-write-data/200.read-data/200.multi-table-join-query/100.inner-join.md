...
内部結合（INNER JOIN）は、2つ以上のテーブルを結合し、結合条件を満たす複数のテーブルデータを返すために使用されます。INNER JOINは、単純結合とも呼ばれます。

## INNER JOIN構文

INNER JOINでは、デフォルトで`JOIN`が返す結果は`ON`句に記述された結合条件を満たすものになります。通常、キーワード`INNER`は省略されます。`JOIN`の前後に記述されるテーブルは、それぞれ左テーブルと右テーブルと呼ばれます。`ON`句では、左テーブルと右テーブルの結合条件およびフィルタリング条件を記述します。
...
`WHERE`句と`ORDER BY`句の両方を同時に使用する場合、SQLが返す結果は`JOIN`の結果を基に、`WHERE`句の検索条件でフィルタリングされ、さらに`ORDER BY`句で指定された列に従ってソートされます。

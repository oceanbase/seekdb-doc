...
システム変数の設定により、seekdbの動作を業務要件に合わせることができます。本記事では、システム変数の変更方法について説明します。
...
以下の例で使用されているシステム変数`ob_query_timeout`は、SQL文のクエリ操作におけるタイムアウト時間を設定するために使用されます。単位はマイクロ秒です。大規模なデータのクエリ操作を実行する際に、システム変数`ob_query_timeout`の値を大きく設定することで、クエリのタイムアウトによる文の実行失敗を防ぐことができます。
...
4. 変更結果を確認します。
    ...
    変更が成功した後、ビュー `DBA_OB_SYS_VARIABLES` を使用して `ob_query_timeout` 変数の詳細な変更履歴とデフォルト値を確認できます。ステートメントは以下のとおりです。
...
ブール型変数の設定には、ON/OFF、TRUE/FALSE、または1/0の3つの方法があり、これらは互換性があります。

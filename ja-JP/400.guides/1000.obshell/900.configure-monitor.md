...
obshell Dashboardのパフォーマンス監視機能はobagentおよびPrometheusツールに依存し、アラート機能はAlertmanagerツールに依存します。obagentはHTTPインターフェースを提供し、seekdbからデータを収集できます。Prometheusは設定に基づいて定期的にobagentのインターフェースにアクセスし、監視データを収集して永続化します。obshell Dashboardは、Prometheusが収集した監視データを表示するページを提供します。Alertmanagerは、Prometheusなどの監視システムからのアラートを処理し、アラートの重複除去、グループ化、ルーティング、サイレンシングなどの機能を提供します。
...
1. ユーザーの準備
   ...
   OceanBaseデータベースの読み取り専用ユーザーを作成し、oceanbase.*に対する読み取り権限のみを付与することを推奨します。
...
2. インスタンス名の設定
   ...
   Prometheusを使用して複数のseekdbインスタンスを監視する場合は、各seekdbインスタンスに異なるインスタンス名を設定し、インスタンスを区別しやすくします。
...
1. obagentのインストール
   ...
   [オープンソースイメージレジストリ](https://mirrors.aliyun.com/oceanbase/community/stable/)にアクセスし、ご利用環境のアーキテクチャに応じてobagentのインストールパッケージをダウンロードして、以下のコマンドでインストールします。最新バージョンのインストールを推奨します。
...
2. obagentの設定
   ...
   コマンドパラメータの説明は以下の通りです：

   * `monagent.ob.monitor.user`：収集アカウントを設定します。seekdbで作成した読み取り専用ユーザー（本記事では`monitor`）を指定します。
   ...
   * `monagent.pipeline.ob.status`：`active`に設定すると、seekdb関連指標の収集が有効になります。
...
[Prometheus公式サイト](https://prometheus.io/)または[オープンソースミラーアカイブ](https://mirrors.aliyun.com/oceanbase/community/stable/)にアクセスし、ご利用の環境のアーキテクチャに応じて対応するインストールパッケージをダウンロードしてPrometheusをデプロイします。通常、すべてのseekdbインスタンスを監視するには、1つのPrometheusインスタンスだけで十分です。このセクションで提供されている設定例は参考用です。具体的なインストールおよび設定の詳細については、[Prometheus公式ドキュメント](https://prometheus.io/docs/prometheus/latest/getting_started/)を参照してください。
...
global:
  ...
  evaluation_interval: 5s # ルールを15秒ごとに評価します。デフォルトは1分です。
...
# rule_filesにルールファイルを一度読み込み、グローバルの'evaluation_interval'に従って定期的に評価します。
rule_files:
...
:::info
   Prometheusのバージョンによって画面の表示が異なります。ここでの<b>Target</b>フィールドはあくまで例です。
:::
...
[Prometheus公式サイト](https://prometheus.io/)または[オープンソースミラーコレクション](https://mirrors.aliyun.com/oceanbase/community/stable/)にアクセスし、使用環境のアーキテクチャに対応するパッケージをダウンロードして解凍します。[Alertmanagerドキュメント](https://prometheus.io/docs/alerting/latest/configuration/)を参照して設定ファイル(`alertmanager.yml`)を修正し、完了後に作業ディレクトリに移動して以下のコマンドを実行してAlertmanagerを起動します。
...
例では、`127.0.0.1:9093`がサービスアドレスを表し、nuohupはalertmanagerプロセスをバックグラウンドで実行することを示しています。
...
* `SQLSTATE [VALUE] sqlstate_value`: a 5-character string literal that represents an `SQLSTATE` value. Do not use `SQLSTATE` values that start with '00', because '00' values indicate success, not an error.

For more information about error codes and `SQLSTATE` values, see [Common error codes of seekdb](../../600.error-code/100.error-code-overview.md).

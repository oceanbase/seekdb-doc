...
<Tabs>
    ...
    * 检查磁盘
    ...
    2. 安装 OceanBase Desktop
        ...
        1. 下载安装包
            ...
            访问 [seekdb 软件下载中心](https://www.oceanbase.ai/zh-CN/download)，找到 **OceanBase Desktop**，选择需要下载的版本、操作系统、CPU 架构，再单击 **下载** 即可下载。
    ...
    seekdb 中存在 3 种日志文件，分别为 `app.log`、`seekdb.log` 和 `obshell.log`。

    * app.log

      `app.log` 日志记录了应用程序的相关操作， 例如启动、关闭以及异常信息等。其日志路径为：`./Users/admin/AppData/Roaming/OceanBase-Desktop/logs/app.log`。

    * seekdb 日志路径：

      `seekdb.log` 日志记录了 seekdb 的相关操作， 例如数据库操作、查询、连接、事务、错误等信息等。其日志路径为：`./Users/admin/AppData/Roaming/OceanBase-Desktop/logs/seekdb.log`。

    * obshell 日志路径：

      `obshell.log` 日志记录了 obshell 的相关操作， 例如 obshell 的连接、操作、错误等信息等。其日志路径为：`./Users/admin/AppData/Roaming/OceanBase-Desktop/logs/obshell.log`。
  ...
  <TabItem value="MacOS 操作系统" label="MacOS 操作系统">
    ...
    * app.log
      ...
      `app.log` 日志记录了应用程序的相关操作， 例如启动、关闭以及异常信息等。其日志路径为：`~/Library/Application Support/OceanBase-Desktop/logs/app.log`。

    * seekdb 日志路径：

      `seekdb.log` 日志记录了 seekdb 的相关操作， 例如数据库操作、查询、连接、事务、错误等信息等。其日志路径为：`/opt/homebrew/var/seekdb/data/log/seekdb.log`。

    * obshell 日志路径：

      `obshell.log` 日志记录了 obshell 的相关操作， 例如 obshell 的连接、操作、错误等信息等。其日志路径为：`/opt/homebrew/var/seekdb/data/log/obshell.log`。

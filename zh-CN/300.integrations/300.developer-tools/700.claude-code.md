...
本文使用 Claude Code，展示如何通过 seekdb Agent Skill，快速构建可被 AI 自动调用的知识包 + 工作流，以及如何通过 seekdb MCP Server 快速构建一个后端应用。

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs>
  <TabItem value="seekdb Agent Skill 与 Claude Code 集成" label="seekdb Agent Skill 与 Claude Code 集成" default>
   ...
   seekdb Agent Skills 是基于该标准构建的「可被 AI 自动调用的知识包 + 工作流」。在 Claude Code 中安装后，当你提问 seekdb 相关问题时，Claude Code 会自动按技能说明查阅 seekdb 文档或执行数据导入导出等操作，无需记住命令或手动复制文档，即可在编码过程中获得 seekdb 的官方文档支持与实操能力。更多关于 Agent Skill 的信息，请参考 [seekdb Agent Skills](../../100.get-started/110.use-seekdb-with-skill/100.agent-skills.md)。
   ...
   ## 在 Claude Code 中安装 seekdb Agent Skills

   Claude Code 通过项目根目录下的 `.claude/skills` 目录识别并加载 Agent Skill。按以下步骤完成安装：
   ...
   4. 选择工具。
      ...
      通过键盘上的 **↑↓** 键选择 **Claude Code**，**Enter** 键确认。
   ...
   ## 在 Claude Code 中使用 seekdb Agent Skills

   安装完成后，在安装了技能的项目根目录下打开 Claude Code，直接提问即可。AI 会自动调用对应技能并基于文档或技能能力作答。
  ...
  <TabItem value="seekdb MCP Server 与 Claude Code 集成" label="seekdb MCP Server 与 Claude Code 集成">

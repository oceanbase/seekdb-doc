...
创建一个名为 `jinaai_oceanbase_demo_documents` 的表，并将向量数据存入 seekdb：
...
client = pyseekdb.Client(host=OCEANBASE_DATABASE_HOST, port=OCEANBASE_DATABASE_PORT, database=OCEANBASE_DATABASE_DB_NAME, user=OCEANBASE_DATABASE_USER, password=OCEANBASE_DATABASE_PASSWORD)
...
通过 Jina AI 嵌入 API 生成查询文本的嵌入向量，然后根据查询文本的嵌入向量与向量表中的每个嵌入向量的cosine距离，搜索最相关的文档：

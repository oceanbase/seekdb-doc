...
* 外键约束（`FOREIGN KEY`）：要求约束的列的值取自于另外一个表的主键列。
  ...
  seekdb 默认开启了外键约束检查，外键约束检查开关变量 `foreign_key_checks` 来控制。`foreign_key_checks` 变量的更多说明，请参见 [foreign_key_checks](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016028)。
...
自增列有三个重要属性，包括自增起始值、自增步长和自增列缓存大小，通过 `auto_increment_cache_size`、`auto_increment_increment` 和 `auto_increment_offset` 三个变量来控制。
